...
интерактивных областях: `1/3` рядом с водителем и `2/3` в дальней части экрана.
...
> Статус: версия `0.3.0`, тестируется на DiLink 5 / Android 12. Перед поездкой
> настройте интерфейс на стоящем автомобиле.

## Скриншоты

| Два окна | Настройки и диагностика |
| --- | --- |
...
- диагностика ADB, касаний, руля и Accessibility Service;
- перезапуск вспомогательных shell-процессов из настроек;
- автозапуск BYD Split после загрузки DiLink;
...
| Специальные возможности: «Кнопка микрофона BYD Split» | Определение открытого чата MAX и обработка кнопки руля | В настройках BYD Split → «Специальные возможности» |
...
6. При первом открытии подтвердите штатный запрос ADB, отметив
   «Всегда разрешать».
7. Откройте модальные настройки и включите службу специальных возможностей,
   если нужна кнопка микрофона для MAX.
...
На протестированной ревизии DiLink 5 используются низкоуровневые события
`scan 312` для долгого и `scan 290` для короткого нажатия:
...
4. Второе короткое нажатие отправляет сообщение.
...
Откройте меню любой панели свайпом вниз и нажмите `⚙`. Модальное окно показывает:

- включён ли ADB;
- доступен ли основной мост;
- работает ли быстрый ввод и мультитач;
- работает ли обработчик руля;
- включена ли служба специальных возможностей;
- включены ли автозапуск и демо-режим.

Кнопка «Разрешить ADB и запустить помощники» инициирует первое системное
подтверждение. Кнопка «Перезапустить помощники» восстанавливает оба процесса
без компьютера.
...
мультитач и независимую смену панелей. Скрипт запускает стандартные Android
ADB-помощники, но не запускает обработчик руля и Accessibility-интеграцию MAX,
поскольку этих BYD-компонентов в AVD нет.
